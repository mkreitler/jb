...
jb.fonts = {
    ...
    print: function(fontName, text, color, hAlign, vAlign, scale) {
        jb.fonts.print(fontName, 0, 0, text, color, hAlign, vAlign, scale);        
    },
    ...
    printAt: function(fontName, newRow, newCol, text, color, hAlign, vAlign, scale) {
        ...
        if (text && charSet) {   
            ...
            y -= scale * this.DEFAULT_SIZE * (0.5 + (hAlign - 0.5)); 
            x -= scale * this.DEFAULT_SIZE * text.length * (0.5 + (vAlign - 0.5));
...
jb.glyphs = {
    draw: function(sizeStr, glyphName, x, y, scaleX, scaleY) {
        ...
        if (jb.ctxt && glyphData) {
            ...
            jb.ctxt.drawImage(glyphData.image[key], x, y);
        }
    },
        ...
        if (pixelData && !glyphImage[key]) {
            ...
            for (iRow=0; iRow<pixelData.length; ++iRow) {
                for (iCol=0; iCol<pixelData[iRow].length; iCol+=2) {
                    iPalette = parseInt(pixelData[iRow].charAt(iCol), 16);
                    iColor = parseInt(pixelData[iRow].charAt(iCol + 1), 16);
                    ...
                    glyphCtxt.fillRect(x, y, Math.abs(scaleX), Math.abs(scaleY));
                }
            }
